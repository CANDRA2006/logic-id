...
export default function Game() {
  ...
  useEffect(() => {
    if (!gameOver || !user || saving) return;
      ...
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-6 pt-20">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md w-full">
          ...
          <div className="bg-white/3 border border-white/8 rounded-2xl p-6 space-y-4 mb-6">
            {[
              { label: g.finalScore, value: score, accent: true },
              { label: g.accuracy, value: `${accuracy}%` },
              { label: g.correct, value: `${correct} / ${questions.length}` },
              { label: g.xpEarned, value: saving ? "..." : `+${results?.xpEarned || 0} XP` },
            ].map((row) => (
              <div key={row.label} className="flex items-center justify-between">
                <span className="text-sm text-zinc-500">{row.label}</span>
                <span className={`font-bold text-sm ${row.accent ? "text-violet-400 text-lg" : "text-white"}`}>{row.value}</span>
              </div>
            ))}
            {results?.newLevel && (
              <div className="border-t border-white/5 pt-4 text-center">
                <p className="text-xs text-zinc-500 mb-1">{g.currentLevel}</p>
                <p className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-cyan-400">Level {results.newLevel}</p>
              </div>
            )}
          </div>
          <div className="flex gap-3">
            <button onClick={() => navigate(`/game/${category}`)}
              className="flex-1 py-3 bg-violet-600 hover:bg-violet-500 text-white font-semibold rounded-xl transition-all text-sm">{g.playAgain}</button>
            <button onClick={() => navigate("/play")}
              className="flex-1 py-3 bg-white/5 hover:bg-white/10 border border-white/8 text-zinc-300 font-medium rounded-xl transition-all text-sm">{g.categories}</button>
          </div>
        </motion.div>
      </div>
    );
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-right">
              ...
              <p className="text-lg font-black text-white">{currentIdx === 0 ? "--" : `${Math.round((correct / currentIdx) * 100)}%`}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-start justify-between gap-6 mb-8">
          <AnimatePresence mode="wait">
            <motion.div key={currentIdx} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }} className="flex-1">
              ...
              <h2 className="text-xl font-bold text-white leading-snug">{currentQ.question}</h2>
            </motion.div>
          </AnimatePresence>
          <div className="flex-shrink-0"><CircularTimer timeLeft={timeLeft} maxTime={20} /></div>
        </div>

        <AnimatePresence mode="wait">
          <motion.div key={currentIdx} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-2 gap-3">
            {currentQ.options.map((opt, i) => {
              let style = "bg-white/3 border-white/8 text-zinc-300 hover:bg-white/6 hover:border-white/15 hover:text-white";
              if (answered) {
                if (i === currentQ.correct) style = "bg-emerald-500/15 border-emerald-500/40 text-emerald-300";
                else if (i === selected && selected !== currentQ.correct) style = "bg-red-500/15 border-red-500/40 text-red-300";
              ...
              return (
                ...
                >
                  <span className="opacity-50 text-xs mr-2">{String.fromCharCode(65 + i)}.</span>{opt}
                </motion.button>
              );
            })}
          </motion.div>
